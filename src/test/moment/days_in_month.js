...
test('days in month of all but february', function (assert) {
    ...
    for (year = 1899; year < 2100; year++) {
        for (month = 0; month < 12; month++) {
            if (month != 1) {
                assert.equal(customMoment([year, month]).daysInMonth(), days[month]);
...
test('days in month', function (assert) {
    each([31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31], function (days, i) {
        var firstDay = customMoment([2012, i]),
            lastDay = customMoment([2012, i, days]);
        assert.equal(
...
test('days in month leap years', function (assert) {
    assert.equal(
        customMoment([2010, 1]).daysInMonth(),
        28,
        'Feb 2010 should have 28 days'
    );
    assert.equal(
        customMoment([2100, 1]).daysInMonth(),
        28,
        'Feb 2100 should have 28 days'
    );
    assert.equal(
        customMoment([2008, 1]).daysInMonth(),
        29,
        'Feb 2008 should have 29 days'
    );
    assert.equal(
        customMoment([2000, 1]).daysInMonth(),
        29,
...
test('days in month with NaN inputs', function (assert) {
    ...
    assert.ok(
        !customMoment([2010, null, null]).isValid(),
